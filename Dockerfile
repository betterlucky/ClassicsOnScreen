...
# Create a user with the same UID and add to sudo group
...
RUN mkdir -p /app/db /app/staticfiles /etc/classicsonscreen \
    && chown -R appuser:appuser /app \
    && chmod 755 /app/staticfiles \
    && chmod 777 /app/db \
    && chown -R appuser:appuser /etc/classicsonscreen \
    && touch /etc/classicsonscreen/env.py \
    && chown appuser:appuser /etc/classicsonscreen/env.py \
    && chmod 600 /etc/classicsonscreen/env.py
